...
-- ╔══════════════════════════════════════════════════════════════╗
-- ║  MRXN PATCH — Built-in ke Uhhhhhh Reanimate                 ║
-- ║  Dance Visible Public | Fly | Noclip | Freeze | Teleport     ║
-- ║  FIXED: Dance tetap jalan saat player baru join              ║
-- ╚══════════════════════════════════════════════════════════════╝
...
task.spawn(function()
	...
	local function GetReanimRoot()
		local c = Reanimate.Character
		return c and c:FindFirstChild("HumanoidRootPart")
	end
	local function GetRealRoot()
		local c = Player.Character
		return c and c:FindFirstChild("HumanoidRootPart")
	end
	local function GetReanimHum()
		local c = Reanimate.Character
		return c and c:FindFirstChildOfClass("Humanoid")
	end
	local function GetRealHum()
		local c = Player.Character
		return c and c:FindFirstChildOfClass("Humanoid")
	end

	-- ════════════════════════════════════════════════
	--  MODUL: DANCE VISIBLE (FIXED - auto-restart & respawn-safe)
	-- ════════════════════════════════════════════════
	local MRXN_Dance = {
		Active      = false,
		AnimId      = "",
	...
	local function MRXN_StopDance()
		...
		MRXN_Dance._Restarting = false
		if MRXN_Dance.Track then
			pcall(function() MRXN_Dance.Track:Stop(0.3) end)
			MRXN_Dance.Track = nil
		end
	end

	local function MRXN_InternalPlay(animId, loop, speed)
		local realHum = GetRealHum()
		if not realHum then return false end
		...
		pcall(function() track:Play(0.3, 1, speed or MRXN_Dance.Speed) end)

		MRXN_Dance.Track  = track
		MRXN_Dance.AnimId = tostring(animId)
		MRXN_Dance.Speed  = speed or MRXN_Dance.Speed
		MRXN_Dance.Loop   = (loop ~= false)
		MRXN_Dance.Active = true
		MRXN_Dance._LastStart = os.clock()
		return true
	end

	local function MRXN_PlayDance(animId, loop, speed)
		MRXN_StopDance()
		if MRXN_InternalPlay(animId, loop, speed) then
			MRXNNotify("Dance ON: " .. tostring(animId))
			return true
		end
		MRXNNotify("Gagal load anim: " .. tostring(animId))
		return false
	end

	-- ─────────────────────────────────────────────
	--  WATCHDOG + SYNC LOOP (selalu hidup)
	-- ─────────────────────────────────────────────
	task.spawn(function()
		while true do
			RunService.Heartbeat:Wait()
			if MRXN_Dance.Active and MRXN_Dance.Track then
				local rc = Reanimate.Character
				local rl = Player.Character

				-- Sinkronisasi Motor6D (biar kelihatan sama orang lain)
				if rc and rl then
					for _, motor in rl:GetDescendants() do
						if motor:IsA("Motor6D") then
							local rm = rc:FindFirstChild(motor.Name, true)
							if rm and rm:IsA("Motor6D") then
								rm.Transform = motor.Transform
							end
						end
					end
				end

				-- Cek apakah track masih main
				local playing = false
				pcall(function() playing = MRXN_Dance.Track.IsPlaying end)

				if not playing and not MRXN_Dance._Restarting then
					MRXN_Dance._Restarting = true
					local aId   = MRXN_Dance.AnimId
					local lLoop = MRXN_Dance.Loop
					local lSpd  = MRXN_Dance.Speed

					task.spawn(function()
						local waitFor = 0.25
						local delta = os.clock() - MRXN_Dance._LastStart
						if delta < waitFor then
							task.wait(waitFor - delta)
						end
						if not MRXN_Dance.Active then
							MRXN_Dance._Restarting = false
							return
						end
						if not GetRealHum() then
							MRXN_Dance.Active = false
							MRXN_Dance._Restarting = false
							return
						end
						if MRXN_InternalPlay(aId, lLoop, lSpd) then
							MRXNNotify("Dance auto-restart")
						else
							MRXN_Dance.Active = false
						end
						MRXN_Dance._Restarting = false
	...
	-- Kalau karakter ke-respawn (misalnya sync pas player baru join),
	-- tunggu bentar lalu lanjutin dance yang sama
	Player.CharacterAdded:Connect(function()
		if MRXN_Dance.Active and MRXN_Dance.AnimId ~= "" then
			local aId   = MRXN_Dance.AnimId
			local lLoop = MRXN_Dance.Loop
			local lSpd  = MRXN_Dance.Speed
			task.spawn(function()
				task.wait(1)
				if MRXN_Dance.Active and GetRealHum() then
					MRXN_InternalPlay(aId, lLoop, lSpd)
					MRXNNotify("Dance resumed setelah respawn")
				end
			end)
		end
	end)
	...
	local function MRXN_StartFly()
		...
		local root = GetReanimRoot() or GetRealRoot()
		...
		MRXNNotify("Fly ON — W/A/S/D gerak, E naik, Q turun")
	end
	...
	local function MRXN_SetNoclip(on)
		...
		if on then
			_NoclipConn = RunService.Stepped:Connect(function()
				for _, char in {Reanimate.Character, Player.Character} do
					if char then
	...
	local function MRXN_SetFreeze(on)
		...
		if on then
			...
			MRXNNotify("Freeze ON — posisi dikunci")
		else
	...
	local function MRXN_SetSpeed(mul)
		MRXN_Move.SpeedMul = mul
		local rh = GetReanimHum()
		if rh then rh.WalkSpeed = 16 * mul end
		local rl = GetRealHum()
		if rl then rl.WalkSpeed = 16 * mul end
	end
	...
	local function MRXN_TeleportTo(name)
		...
		if not target or not target.Character then
			MRXNNotify("Player tidak ketemu: " .. name) return
		end
		local tr = target.Character:FindFirstChild("HumanoidRootPart")
			   or target.Character:FindFirstChild("LowerTorso")
			   or target.Character:FindFirstChild("Torso")
		if not tr then MRXNNotify("Root target tidak ada") return end
		local dest = tr.CFrame * CFrame.new(0, 0, 3)
		local rc = GetReanimRoot()
		local rl = GetRealRoot()
		if rc then rc.CFrame = dest rc.AssemblyLinearVelocity = Vector3.zero end
		if rl then rl.CFrame = dest rl.AssemblyLinearVelocity = Vector3.zero end
		MRXNNotify("Teleport → " .. target.DisplayName)
	end

	-- ════════════════════════════════════════════════
	--  UI — Halaman MRXN di dalam window Uhhhhhh
	-- ════════════════════════════════════════════════
	UI.CreateSeparator(MainPage)
	UI.CreateText(MainPage, "<b>▼ MRXN EXTRAS ▼</b>", 15, Enum.TextXAlignment.Center)
	UI.CreateSeparator(MainPage)

	UI.CreateText(MainPage, "◈ DANCE VISIBLE (orang lain bisa lihat)", 13, Enum.TextXAlignment.Left)

	local danceAnimBox = UI.CreateTextbox(MainPage, "", "AnimationId custom...", 16)

	UI.CreateButton(MainPage, "▶ Play Custom AnimId", 18).Activated:Connect(function()
		local id = danceAnimBox.Text:match("%d+")
		if id then MRXN_PlayDance(id)
		else MRXNNotify("Masukkan AnimationId dulu!") end
	end)

	UI.CreateButton(MainPage, "■ Stop Dance", 18).Activated:Connect(function()
		MRXN_StopDance()
		MRXNNotify("Dance dihentikan")
	end)
	...
	local function OpenDancePage()
		DancePage.Interactable = false
		DancePage.Visible = true
		MainPage.Interactable = false
		local tw = TweenService:Create(DancePage, TweenInfo.new(0.5, Enum.EasingStyle.Cubic, Enum.EasingDirection.In), {
			Position = UDim2.new(0.5, 0, 0.5, 0)
		})
		tw:Play()
		tw.Completed:Connect(function() DancePage.Interactable = true end)
	end
	local function CloseDancePage()
		DancePage.Interactable = false
		local tw = TweenService:Create(DancePage, TweenInfo.new(0.5, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {
			Position = UDim2.new(0.5, 360, 0.5, 0)
		})
		tw:Play()
	...
	UI.CreateButton(DancePage, "< Kembali", 18).Activated:Connect(CloseDancePage)
	UI.CreateText(DancePage, "Pilih Dance — terlihat semua orang", 13, Enum.TextXAlignment.Center)
	...
	UI.CreateButton(MainPage, "Pilih Dance Preset ▶", 18).Activated:Connect(OpenDancePage)

	-- ── Movement Section ──
	UI.CreateSeparator(MainPage)
	UI.CreateText(MainPage, "◈ POSISI BEBAS", 13, Enum.TextXAlignment.Left)

	local flySwitch = UI.CreateSwitch(MainPage, "Fly Mode (E naik, Q turun)", false)
	flySwitch.Changed:Connect(function(v)
		if v then MRXN_StartFly() else MRXN_StopFly() end
	end)

	local flySpeedSlider = UI.CreateSlider(MainPage, "Fly Speed", MRXN_Move.FlySpeed, 5, 300, 5)
	flySpeedSlider.Changed:Connect(function(v)
		MRXN_Move.FlySpeed = v
	end)

	local noclipSwitch = UI.CreateSwitch(MainPage, "Noclip (tembus semua)", false)
	noclipSwitch.Changed:Connect(function(v)
		MRXN_SetNoclip(v)
	end)

	local freezeSwitch = UI.CreateSwitch(MainPage, "Freeze Posisi", false)
	freezeSwitch.Changed:Connect(function(v)
		MRXN_SetFreeze(v)
	end)

	local speedSlider = UI.CreateSlider(MainPage, "Walk Speed Multiplier", 1, 1, 20, 0.5)
	speedSlider.Changed:Connect(function(v)
		MRXN_SetSpeed(v)
	end)
	...
	local tpBox = UI.CreateTextbox(MainPage, "", "Nama player (sebagian)...", 16)
	UI.CreateButton(MainPage, "⚡ Teleport ke Player", 18).Activated:Connect(function()
		MRXN_TeleportTo(tpBox.Text)
	end)

	local TpListPage = UI.CreateItemListPage()
	TpListPage.ZIndex     = 1
	TpListPage.Position   = UDim2.new(0.5, 360, 0.5, 0)
	TpListPage.Interactable = false
	TpListPage.Visible    = false

	local function RebuildTpList()
		for _, c in TpListPage.List:GetChildren() do
		...
		for _, p in Players:GetPlayers() do
			if p ~= Player then
				count += 1
				local item = UI.CreateItemListItem(TpListPage.List)
				UI.CreateText(item, p.DisplayName .. " (@" .. p.Name .. ")", 15, Enum.TextXAlignment.Left)
				local game_name = p.Character and p.Character.Parent and "In World" or "No Char"
				UI.CreateText(item, game_name, 11, Enum.TextXAlignment.Left)
				item.Activated:Connect(function()
					MRXN_TeleportTo(p.Name)
					local tw = TweenService:Create(TpListPage, TweenInfo.new(0.5, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {
						Position = UDim2.new(0.5, 360, 0.5, 0)
					})
					tw:Play()
					tw.Completed:Connect(function()
		...
		if count == 0 then
			UI.CreateText(TpListPage.List, "Tidak ada player lain di server", 14, Enum.TextXAlignment.Center)
		end
	end

	UI.CreateButton(TpListPage, "< Kembali", 18).Activated:Connect(function()
		local tw = TweenService:Create(TpListPage, TweenInfo.new(0.5, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {
			Position = UDim2.new(0.5, 360, 0.5, 0)
		})
		tw:Play()
	...
	UI.CreateText(TpListPage, "Pilih player untuk teleport", 13, Enum.TextXAlignment.Center)
	UI.CreateSeparator(TpListPage)

	UI.CreateButton(MainPage, "Pilih Player ▶", 18).Activated:Connect(function()
		RebuildTpList()
		TpListPage.Interactable = false
		TpListPage.Visible = true
		MainPage.Interactable = false
		local tw = TweenService:Create(TpListPage, TweenInfo.new(0.5, Enum.EasingStyle.Cubic, Enum.EasingDirection.In), {
			Position = UDim2.new(0.5, 0, 0.5, 0)
		})
		tw:Play()
		tw.Completed:Connect(function() TpListPage.Interactable = true end)
	end)

	UI.CreateButton(MainPage, "↩ TP ke 0,0,0 (origin)", 18).Activated:Connect(function()
		local dest = CFrame.new(0, 5, 0)
		local rc = GetReanimRoot()
		local rl = GetRealRoot()
		if rc then rc.CFrame = dest; rc.AssemblyLinearVelocity = Vector3.zero end
		if rl then rl.CFrame = dest; rl.AssemblyLinearVelocity = Vector3.zero end
		MRXNNotify("TP ke origin 0,0,0")
	end)

	UI.CreateSeparator(MainPage)
	UI.CreateText(MainPage, "MRXN Patch by Ghost | v1.0 (FIXED)", 11, Enum.TextXAlignment.Right)
	...
	MRXNNotify("MRXN Extras siap! Scroll ke bawah di menu utama.")
end)
