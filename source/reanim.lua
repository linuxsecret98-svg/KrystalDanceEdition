...
local function KDE_StartServerSync(figure)
	...
	if root then
		...
		pcall(sethiddenproperty, Player, "SimulationRadius", 1000 * #game:GetService("Players"):GetPlayers())
	end
	-- Keep ownership setiap 0.1 detik supaya tidak di-reset game
	_KDE_SyncConnection = task.spawn(function()
		while _KDE_SyncActive do
			if root and root:IsDescendantOf(workspace) then
				pcall(function() root:SetNetworkOwner(Player) end)
				pcall(sethiddenproperty, Player, "SimulationRadius", 1000 * #game:GetService("Players"):GetPlayers())
				-- Reset semua joint supaya replicate ulang
				if figure then
					for _, v in ipairs(figure:GetDescendants()) do
						if v:IsA("Motor6D") then
							pcall(sethiddenproperty, v, "ReplicateCurrentOffset6D", v.Transform.Position)
							pcall(sethiddenproperty, v, "ReplicateCurrentAngle6D", Vector3.new(v.Transform:ToEulerAngles()))
						end
					end
				end
			end
...
SaveData.Reanimator.LimbReplicateFPS10 = SaveData.Reanimator.LimbReplicateFPS10 ~= false and true or false -- KDE: default ON
