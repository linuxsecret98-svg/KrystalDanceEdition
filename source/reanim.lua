...
SaveData.NoLoadAnimationHook = false -- always hook by default for max game compatibility
...
function LimbReanimator.Start()
	...
	local CharConn = Player.CharacterAdded:Connect(function(character)
		...
		-- destroy existing Animator immediately
		local stupid = humanoid and humanoid:FindFirstChildWhichIsA("Animator")
		if stupid then stupid:Destroy() end

		-- destroy Animate script immediately
		stupid = character:FindFirstChild("Animate")
		if stupid then stupid:Destroy() end

		-- aggressive loop: keep killing Animator and Animate every time the game re-adds them
		-- this makes reanimate work on modern games that auto-respawn these
		task.spawn(function()
			local aliveconn
			aliveconn = character.DescendantAdded:Connect(function(v)
				if v:IsA("Animator") then
					task.defer(function() pcall(function() v:Destroy() end) end)
				end
			end)
			character.ChildAdded:Connect(function(v)
				if v.Name == "Animate" and (v:IsA("LocalScript") or v:IsA("Script")) then
			...
			-- poll loop as extra fallback for games that bypass ChildAdded
			while character:IsDescendantOf(game) do
				...
				if animscript then pcall(function()
					animscript.Disabled = true
					animscript:Destroy()
				end) end
				task.wait(0.5)
			end
			if aliveconn then aliveconn:Disconnect() end
		end)
	end)
...
function HatReanimator.Start()
	...
	local function OnCharacter(character)
		...
		-- always kill Animate and Animator regardless of hook setting (max game compat)
		do
			local stupid = character:FindFirstChild("Animate")
			if stupid then stupid:Destroy() end
			stupid = Humanoid:FindFirstChildWhichIsA("Animator")
			if stupid then stupid:Destroy() end
			character.ChildAdded:Connect(function(v)
				if v.Name == "Animate" and (v:IsA("LocalScript") or v:IsA("Script")) then
					v.Disabled = true
