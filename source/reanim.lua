...
Reanimate.CreateCharacter = function(InitCFrame)
	...
	Util.LinkDestroyI2C(RC, RunService.PreAnimation:Connect(function(dt)
		...
		-- KDE: tidak ada teleport paksa, bebas ke mana saja
	end))
...
Util.SetMotor6DTransform = function(motor, transform)
	...
	local axis, ang2 = transform:ToAxisAngle()
	local newangle = axis * ang2
...
SaveData.Reanimator.LimbMode = SaveData.Reanimator.LimbMode or 2 -- KDE: default Keep RootPart Streamed
...
function LimbReanimator.Start()
	...
	-- KDE: auto-start server visibility
	if _G_Uhhhhhh and _G_Uhhhhhh.ServerVisibility then
		_G_Uhhhhhh.ServerVisibility.Start()
	end
	while not Reanimate.Stopping do
		RunService.PreSimulation:Wait()
		workspace.FallenPartsDestroyHeight = -999999 -- KDE: parts tidak pernah dihapus
		...
		-- KDE: rootcf mulai dari posisi character asli, bisa gerak bebas
		local _realRoot = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
		local rootcf = _realRoot and _realRoot.CFrame or CFrame.new(rootposition)
		...
		if ReanimCharacter then
			...
			if Character and Humanoid and RootPart then
				...
				if RCRootPart and RCTorso then
					-- KDE: auto-detect mode terbaik
					local _m = LimbReanimator.Mode
					if workspace.StreamingEnabled then _m = 2 end
					if RCRootPart and RCRootPart.Position.Y < -500 then _m = 1 end
					if _m == 1 then
						rootcf = CFrame.new(rootposition2)
					end
					if _m == 2 or workspace.StreamingEnabled then
...
function HatReanimator.Start()
	...
	local function SetSimulationRadius()
		local function setsimrad(plr, radius)
			...
			pcall(sethiddenproperty, plr, "MaximumSimulationRadius", radius)
		end
		-- KDE: radius sangat besar, cover semua player berapapun jumlahnya
		setsimrad(Player, _KDE_MAX_RADIUS)
		for _, plr in ipairs(Players:GetPlayers()) do
			if plr ~= Player then
				setsimrad(plr, _KDE_MAX_RADIUS)
			end
		end
	end
	...
	local function SetUACFrameNetless(handle, dt, newcf, tvel, fling, spin)
		...
		-- KDE: no Y limit, bisa di mana saja
		...
		if not handle:IsGrounded() and IsNetworkOwner(handle) then
			...
			handle.CFrame = newcf
			-- KDE: force ownership setiap set CFrame supaya keliatan di server
			pcall(sethiddenproperty, handle, "NetworkOwnershipRule", Enum.NetworkOwnership.Manual)
			pcall(sethiddenproperty, handle, "NetworkOwner", Player)
			if spin then
...
-- ============================================================
-- KDE SERVER VISIBILITY ENGINE
-- Gerakan keliatan di semua game, semua jumlah player
-- ============================================================
local KDE_SV = {}
KDE_SV.Active = false
KDE_SV.MaxRadius = 2^24

local function KDE_ForceOwn(part)
	...
	pcall(sethiddenproperty, part, "NetworkOwner", Player)
end

local function KDE_SetRadius()
	pcall(function() Player.SimulationRadius = KDE_SV.MaxRadius end)
	pcall(sethiddenproperty, Player, "SimulationRadius", KDE_SV.MaxRadius)
	pcall(sethiddenproperty, Player, "MaximumSimulationRadius", KDE_SV.MaxRadius)
	for _, plr in ipairs(Players:GetPlayers()) do
		if plr ~= Player then
			pcall(sethiddenproperty, plr, "SimulationRadius", KDE_SV.MaxRadius)
			pcall(sethiddenproperty, plr, "MaximumSimulationRadius", KDE_SV.MaxRadius)
		end
	end
end

local function KDE_AntiStream()
	if not workspace.StreamingEnabled then return end
	local fakeChar = Reanimate.Character
	local realChar = Player.Character
	if not fakeChar or not realChar then return end
	...
	if fakeRoot and realRoot then
		if (fakeRoot.Position - realRoot.Position).Magnitude > 256 then
			fakeRoot.CFrame = realRoot.CFrame * CFrame.new(0, -3, 0)
			fakeRoot.Velocity = Vector3.zero
		end
	end
end

local function KDE_ReplicateChar()
	local char = Player.Character
	if not char then return end
	-- Force ownership semua BasePart karakter
	for _, v in ipairs(char:GetDescendants()) do
		if v:IsA("BasePart") then
			KDE_ForceOwn(v)
		end
	end
	-- Force ownership accessories
	for _, hat in ipairs(char:GetChildren()) do
		if hat:IsA("Accessory") then
			local handle = hat:FindFirstChild("Handle")
			if handle then
				KDE_ForceOwn(handle)
				pcall(function() Player.ReplicationFocus = handle end)
			end
		end
	end
	KDE_AntiStream()
...
local function KDE_PhysicsGlue()
	if not Reanimate.UsePhysicsRepRootPart then return end
	local char = Player.Character
	local fakeChar = Reanimate.Character
	...
	if realRoot and fakeRoot then
		pcall(sethiddenproperty, realRoot, "PhysicsRepRootPart", fakeRoot)
	end
end

function KDE_SV.Start()
	if KDE_SV.Active then return end
	KDE_SV.Active = true
	-- Loop radius + replication tiap 20hz
	task.spawn(function()
		while KDE_SV.Active do
			pcall(KDE_SetRadius)
			pcall(KDE_ReplicateChar)
			task.wait(1/20)
		end
	end)
	-- Physics glue tiap frame
	task.spawn(function()
		while KDE_SV.Active do
			RunService.PreSimulation:Wait()
			pcall(KDE_PhysicsGlue)
		end
	end)
	-- Re-klaim setiap player baru masuk
	Players.PlayerAdded:Connect(function()
		task.wait(1)
		KDE_SetRadius()
	end)
	-- Re-klaim setiap respawn
	Player.CharacterAdded:Connect(function(char)
		task.wait(0.5)
		local root = char:FindFirstChild("HumanoidRootPart")
		if root then KDE_ForceOwn(root) end
		KDE_SetRadius()
	end)
end

function KDE_SV.Stop()
	KDE_SV.Active = false
end

_G_Uhhhhhh.ServerVisibility = KDE_SV
