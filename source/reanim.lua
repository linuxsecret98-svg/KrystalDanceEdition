...
function LimbReanimator.Start()
	...
	local lastspawn = 0
	-- KDE: auto update radius saat player baru masuk
		...
		if ReanimCharacter then
			...
			if Character and Humanoid and RootPart then
				...
				if RCRootPart and RCTorso then
					-- KDE: Auto-select best RootPart mode for current game
					local _mode = LimbReanimator.Mode
					-- StreamingEnabled games: paksa Mode 2
					if workspace.StreamingEnabled then
						_mode = 2
					-- Game yang reset RootPart ke void: paksa Mode 1
					elseif RCRootPart and RCRootPart.Position.Y < -500 then
						_mode = 1
					end
					if _mode == 0 then
						-- very void: default, rootcf tidak berubah
					end
					if _mode == 1 then
						rootcf = CFrame.new(rootposition2)
					end
					if _mode == 2 or workspace.StreamingEnabled then
...
function HatReanimator.Start()
	...
	local function SetSimulationRadius()
		...
		-- KDE: radius besar supaya keliatan berapapun jumlah player
		local r = 2^24
		setsimrad(Player, r)
		-- Set ke semua player supaya mereka bisa lihat kita
	...
	local function SetUACFrameNetless(handle, dt, newcf, tvel, fling, spin)
		...
		if not handle:IsGrounded() and IsNetworkOwner(handle) then
			...
			handle.CFrame = newcf
			-- KDE ServerVisibility: force ownership setiap set CFrame
			if _G_Uhhhhhh and _G_Uhhhhhh.ServerVisibility and _G_Uhhhhhh.ServerVisibility.Enabled then
...
-- ============================================
-- SERVER VISIBILITY LAYER v1.0
-- ============================================
local ServerVisibility = {}
ServerVisibility.Enabled = true
ServerVisibility.HeartbeatRate = 1/20
ServerVisibility.MaxSimulationRadius = 2^24
ServerVisibility.StreamingBuffer = 256
ServerVisibility.ForceNetworkOwnership = true
ServerVisibility.AntiCulling = true

local SV = ServerVisibility
...
local function UpdateSimulationRadius()
	local radius = SV.MaxSimulationRadius
	pcall(function()
		Player.SimulationRadius = radius
		sethiddenproperty(Player, "SimulationRadius", radius)
		sethiddenproperty(Player, "MaximumSimulationRadius", radius)
	end)
	for _, plr in ipairs(Players:GetPlayers()) do
		if plr ~= Player then
			pcall(sethiddenproperty, plr, "SimulationRadius", radius)
...
local function AntiStream(fakeChar, realChar)
	...
	if fakeRoot and realRoot then
		local dist = (fakeRoot.Position - realRoot.Position).Magnitude
		if dist > SV.StreamingBuffer then
...
local function ReplicateHats()
	...
	if SV.AntiCulling then
		AntiStream(fakeChar, char)
	end
	for _, hat in ipairs(char:GetChildren()) do
		if hat:IsA("Accessory") then
			local handle = hat:FindFirstChild("Handle")
			if handle and handle:IsA("BasePart") then
				if handle.Position.Y < workspace.FallenPartsDestroyHeight + 10 then
					local fakeRoot = fakeChar:FindFirstChild("HumanoidRootPart")
					if fakeRoot then
						handle.CFrame = fakeRoot.CFrame
						handle.Velocity = Vector3.new(0, 25, 0)
					end
				end
				ForceOwnership(handle)
...
local function SpoofCharacterIdentity()
	local fakeChar = Reanimate.Character
	if not fakeChar then return end
	fakeChar.Name = Player.Name
	local fakeHum = fakeChar:FindFirstChildOfClass("Humanoid")
	if fakeHum then
		fakeHum.DisplayName = Player.DisplayName
		fakeHum.Name = "Humanoid"
	end
end

local function ApplyPhysicsGlue()
	...
	if realRoot and fakeRoot then
		...
		for _, hat in ipairs(char:GetChildren()) do
			if hat:IsA("Accessory") then
				local handle = hat:FindFirstChild("Handle")
				if handle and handle:GetAttribute("_Uhhhhhh_HasCollide") then
					pcall(sethiddenproperty, handle, "PhysicsRepRootPart", fakeRoot)
				end
			end
		end
	end
...
function SV.Start()
	if not SV.Enabled then SV.Enabled = true end
	task.spawn(function()
		while SV.Enabled do
			pcall(ReplicateHats)
			task.wait(SV.HeartbeatRate)
		end
	end)
	task.spawn(function()
		while SV.Enabled do
			pcall(ApplyPhysicsGlue)
			task.wait(1/60)
		end
	end)
	Player.CharacterAdded:Connect(function()
		task.wait(1)
		SpoofCharacterIdentity()
	end)
end
...
_G_Uhhhhhh = _G_Uhhhhhh or {}
_G_Uhhhhhh.ServerVisibility = SV
