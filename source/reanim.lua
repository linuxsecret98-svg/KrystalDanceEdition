...
do
	...
	if theresassetsmissing then
		...
		task.wait(0.5)
		local s, assetsof = pcall(game.HttpGet, game, "https://api.github.com/repos/linuxsecret98-svg/KrystalDanceEdition/contents/uiassets/")
...
-- SERVER SYNC ENGINE (Krystal Dance Edition)
-- Klaim network ownership karakter supaya joint replicate ke semua game
local _KDE_SyncActive = false
local _KDE_PlayerConn = nil

-- SimulationRadius besar fixed - cover semua player berapapun jumlahnya
local _KDE_MAX_RADIUS = 2^24

local function KDE_SetRadius()
	pcall(sethiddenproperty, Player, "SimulationRadius", _KDE_MAX_RADIUS)
	pcall(sethiddenproperty, Player, "MaximumSimulationRadius", _KDE_MAX_RADIUS)
	-- Set di semua player juga supaya mereka bisa lihat kita
	for _, plr in ipairs(game:GetService("Players"):GetPlayers()) do
		if plr ~= Player then
			pcall(sethiddenproperty, plr, "SimulationRadius", _KDE_MAX_RADIUS)
			pcall(sethiddenproperty, plr, "MaximumSimulationRadius", _KDE_MAX_RADIUS)
		end
	end
end

local function KDE_StartServerSync(figure)
	if _KDE_SyncActive then return end
	_KDE_SyncActive = true

	-- Stop default animator supaya tidak override joint
	...
	-- Claim network ownership + set radius besar
	...
	-- Auto-update radius saat player baru masuk server
	_KDE_PlayerConn = game:GetService("Players").PlayerAdded:Connect(function()
		KDE_SetRadius()
	...
	-- Keep ownership + radius setiap PreSimulation (tiap frame)
	task.spawn(function()
		while _KDE_SyncActive do
			game:GetService("RunService").PreSimulation:Wait()
			local char = Player.Character
			if char then
				local r = char:FindFirstChild("HumanoidRootPart")
				if r and r:IsDescendantOf(workspace) then
					pcall(function() r:SetNetworkOwner(Player) end)
				end
			end
			KDE_SetRadius()
		end
...
local _KDE_CharConn = nil
local function KDE_StopServerSync()
	_KDE_SyncActive = false
	if _KDE_PlayerConn then
		_KDE_PlayerConn:Disconnect()
		_KDE_PlayerConn = nil
	end
	if _KDE_CharConn then
		_KDE_CharConn:Disconnect()
		_KDE_CharConn = nil
	end
end

-- Hook CharacterAdded: setiap respawn langsung klaim ownership lagi
-- Ini fix untuk game seperti Jujutsu Shenanigans yang reset karakter tiap respawn
_KDE_CharConn = Player.CharacterAdded:Connect(function(char)
	task.wait(0.1) -- tunggu sebentar sampai karakter fully loaded
	local root = char:WaitForChild("HumanoidRootPart", 5)
	if root then
		pcall(function() root:SetNetworkOwner(Player) end)
	end
	KDE_SetRadius()
	-- Stop default animator game supaya tidak override joint kita
...
Util.SetMotor6DTransform = function(motor, transform)
	...
	-- KDE: paksa replicate via CurrentAngle (backup method)
	pcall(sethiddenproperty, motor, "CurrentAngle", angle)
end
...
SaveData.Reanimator.LimbMode = SaveData.Reanimator.LimbMode or 2 -- KDE: default Keep RootPart Streamed (kompatibel semua game)
...
SaveData.Reanimator.LimbReplicateFPS10 = false -- KDE: OFF supaya replication tiap frame, bukan cuma 10fps
...
function LimbReanimator.Start()
	...
	local function UpdateTransforms(ReanimCharacter, RootPart, rootcf, rootvel, flingtarget, flingcf)
		...
		local dorep = true
		-- KDE: Selalu replicate tiap frame supaya smooth dan tidak putus
		-- ReplicateFPS10 toggle tetap tersedia tapi default OFF
		...
		if ReanimCharacter then
			...
			if Character and Humanoid and RootPart then
				...
				if RCRootPart and RCTorso then
					-- KDE: Auto-detect RootPart mode terbaik untuk game ini
					local _kdeMode = LimbReanimator.Mode
					if _kdeMode == 0 and workspace.StreamingEnabled then
						_kdeMode = 2 -- force Keep RootPart Streamed kalau game pakai streaming
					end
					if _kdeMode == 1 then
						rootcf = CFrame.new(rootposition2)
					end
					if _kdeMode == 2 or workspace.StreamingEnabled then
...
local function AssetDownload(filename)
	local source = "https://raw.githubusercontent.com/linuxsecret98-svg/KrystalDanceEdition/main/content/" .. filename
	...
	if source:sub(1, 7) == "MARKET/" then
		source = "https://raw.githubusercontent.com/linuxsecret98-svg/KrystalDanceEdition/main/community/" .. source:sub(8)
	end
			...
			if CurrentMovementStyle then
				if ReanimCharacter:GetAttribute("MovementInit") then
					...
					if CurrentDance ~= _CurrentDance then
						...
						-- KDE: Stop server sync saat dance berhenti
						KDE_StopServerSync()
						end
					...
					if _CurrentDance then
						if ReanimCharacter:GetAttribute("IsDancing") then
							...
							if AssetEnsure(_CurrentDance.Assets) then
								...
								-- KDE: Aktifkan server sync sebelum dance dimulai
								KDE_StartServerSync(ReanimCharacter)
...
UI.CreateText(CreditsPage, "If you want to add content to Uhhhhhh, like Dances or Movesets, go to <font color=\"#4444FF\">this thing</font>.", 15, Enum.TextXAlignment.Center).InputBegan:Connect(function(input)
	if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
		...
		pcall(setclipboard, "https://github.com/linuxsecret98-svg/KrystalDanceEdition/")
	end
end)
UI.CreateSeparator(CreditsPage)
...
task.spawn(function()
	...
	xpcall(function()
		local logs = game:HttpGet("https://raw.githubusercontent.com/linuxsecret98-svg/KrystalDanceEdition/main/CHANGELOGS")
...
local function _contentgetgithubraw(path)
	...
	local s, resp = pcall(request, {
		Method = "GET",
		Url = "https://api.github.com/repos/linuxsecret98-svg/KrystalDanceEdition/contents/content/" .. path,
		Headers = {
	...
	s, resp = pcall(request, {
		Method = "GET",
		Url = "https://raw.githubusercontent.com/linuxsecret98-svg/KrystalDanceEdition/main/content/" .. path,
	})
	if s and resp and resp.StatusCode == 200 then
		return resp.Body
	end
...
local function ForceModuleReload(force)
	...
	if force ~= "SKIPHASH" then
		xpcall(function()
			local s, resp = pcall(request, {
				Method = "GET",
				Url = "https://api.github.com/repos/linuxsecret98-svg/KrystalDanceEdition/contents/content/",
			})
			if s and resp and resp.StatusCode == 200 then
...
--[[local function _getgithubraw(path)
	local s, resp = pcall(request, {
		Method = "GET",
		Url = "https://api.github.com/repos/linuxsecret98-svg/KrystalDanceEdition/contents/" .. path,
		Headers = {
			Accept = "application/vnd.github.VERSION.raw"
		}
	})
	...
	s, resp = pcall(request, {
		Method = "GET",
		Url = "https://raw.githubusercontent.com/linuxsecret98-svg/KrystalDanceEdition/main/" .. path,
	})
	if s and resp and resp.StatusCode == 200 then
		return resp.Body
	end
...
local function GetMarketList()
	...
	if not aitemus or not file2name or not file2aitemu then
		local marketteresult = game:HttpGet("https://raw.githubusercontent.com/linuxsecret98-svg/KrystalDanceEdition/main/community/list.txt")
		...
		for _,a in marketteresult do
			if #a > 3 then
				...
				if k == "file" then
					aitemu.Source = "https://raw.githubusercontent.com/linuxsecret98-svg/KrystalDanceEdition/main/community/" .. v
