...
-- ╔══════════════════════════════════════════════════════════════╗
-- ║  MRXN PATCH v2.0 — Built-in ke Uhhhhhh Reanimate            ║
-- ║  Dance Visible (fix replikasi) | Multi-game | Posisi Bebas  ║
-- ╚══════════════════════════════════════════════════════════════╝
...
task.spawn(function()
	...
	--  UTILS
	-- ════════════════════════════
	local function MRXNNotify(msg)
		Util.UINotify("[MRXN] " .. msg)
	end
	...
	-- ════════════════════════════════════════════════════════════
	--  MODUL DANCE VISIBLE — FIX LENGKAP
	--
	--  Masalah lama:
	--   1. Loop sync pakai `track.IsPlaying` → mati kalau track pause sebentar
	--   2. Tidak ada restart otomatis kalau track di-stop oleh game
	--   3. Player baru join tidak dapat replikasi
	--
	--  Fix v2:
	--   • Pakai Heartbeat connection PERMANEN (tidak bergantung track.IsPlaying)
	--   • Track .Stopped → auto restart
	--   • HumanoidLASetHookState(false) sebelum LoadAnimation → bypass hook Uhhhhhh
	--     sehingga track benar-benar jalan di real character (bukan dummy)
	--   • Motor6D sync jalan terus selama MRXN_Dance.Active = true,
	--     sehingga player baru join pun langsung dapat pose yang sedang berjalan
	-- ════════════════════════════════════════════════════════════
	...
	local MRXN_Dance = {
		...
		Track    = nil,
		Speed    = 1,
		SyncConn = nil,  -- Heartbeat connection sync Motor6D
	}
	...
	-- Hentikan sync Motor6D
	local function MRXN_StopSyncLoop()
		if MRXN_Dance.SyncConn then
			MRXN_Dance.SyncConn:Disconnect()
			MRXN_Dance.SyncConn = nil
		end
	end
	...
	local function MRXN_StopDance()
		...
		if MRXN_Dance.Track then
			pcall(function() MRXN_Dance.Track:Stop(0.2) end)
			MRXN_Dance.Track = nil
		end
	end

	-- Mulai Motor6D sync loop PERMANEN
	-- Loop ini jalan terus selama Active=true, TIDAK bergantung track.IsPlaying
	-- Ini yang bikin player baru join pun langsung lihat pose yang sedang berjalan
	local function MRXN_StartSyncLoop()
		MRXN_StopSyncLoop()
		MRXN_Dance.SyncConn = RunService.Heartbeat:Connect(function()
			...
			local rc = GetRC()
			local rl = GetRL()
			if not rc or not rl then return end
			-- Copy setiap Motor6D dari real char ke reanimate char
	...
	-- Internal: load dan play track di real character
	-- Menonaktifkan hook LoadAnimation Uhhhhhh sementara agar track
	-- benar-benar diputar di real char, bukan dialihkan ke dummy
	local function MRXN_LoadAndPlayTrack(animId)
		local realHum = GetRLHum()
		if not realHum then return nil end

		-- Matikan hook sementara supaya LoadAnimation jalan normal
		HumanoidLASetHookState(false)

		local anim = Instance.new("Animation")
		anim.AnimationId = "rbxassetid://" .. tostring(animId)
		local ok, track = pcall(function() return realHum:LoadAnimation(anim) end)
		anim:Destroy()

		-- Aktifkan kembali hook
		HumanoidLASetHookState(Reanimate.UseLoadAnimationHook)
		...
		-- Auto-restart kalau track di-stop oleh game (anti-override)
		track.Stopped:Connect(function()
			if MRXN_Dance.Active and MRXN_Dance.AnimId == tostring(animId) then
				task.delay(0.05, function()
					if MRXN_Dance.Active and MRXN_Dance.AnimId == tostring(animId) then
						-- Reload track baru karena track lama sudah stop
						local newTrack = MRXN_LoadAndPlayTrack(animId)
						if newTrack then
							MRXN_Dance.Track = newTrack
						end
					end
				end)
			end
		...
		track:Play(0.2, 1, MRXN_Dance.Speed)
		return track
	end
	...
	local function MRXN_PlayDance(animId)
		...
		if not GetRLHum() then
			MRXNNotify("Reanimate dulu, baru dance!")
			return false
		end

		MRXN_Dance.AnimId  = tostring(animId)
		MRXN_Dance.Active  = true

		local track = MRXN_LoadAndPlayTrack(animId)
		if not track then
			MRXN_Dance.Active = false
			MRXNNotify("Gagal load animasi: " .. tostring(animId))
			return false
		end
		MRXN_Dance.Track = track

		-- Start sync loop PERMANEN — player baru join pun dapat pose ini
		MRXN_StartSyncLoop()

		-- Guard: pastikan track tetap playing tiap 2 detik
		-- Ini handle game yang terus-terusan paksa stop animasi
		task.spawn(function()
			while MRXN_Dance.Active and MRXN_Dance.AnimId == tostring(animId) do
				task.wait(2)
				if MRXN_Dance.Active and MRXN_Dance.Track then
					if not MRXN_Dance.Track.IsPlaying then
						pcall(function() MRXN_Dance.Track:Play(0.1, 1, MRXN_Dance.Speed) end)
					end
				end
			end
		end)

		MRXNNotify("Dance ON  (ID: " .. tostring(animId) .. ")")
		return true
	end
	...
	local _FlyBV, _FlyBG, _FlyConn = nil, nil, nil
	local _NoclipConn  = nil
	local _FreezeConn  = nil
	local _FrozenCF    = nil

	local function MRXN_DestroyFlyParts()
	...
	local function MRXN_StartFly()
		MRXN_DestroyFlyParts()
		local root = GetRCRoot() or GetRLRoot()
		if not root then MRXNNotify("Reanimate dulu baru fly!") return end
		...
		_FlyConn = RunService.RenderStepped:Connect(function()
			if not MRXN_Move.FlyEnabled then MRXN_DestroyFlyParts() return end
			local r = GetRCRoot() or GetRLRoot()
			...
			local ks  = {}
			for _, k in UserInputService:GetKeysPressed() do ks[k.KeyCode] = true end
		...
		MRXNNotify("Fly ON — WASD, E naik, Q turun")
	end

	local function MRXN_StopFly()
		MRXN_Move.FlyEnabled = false
		MRXN_DestroyFlyParts()
		MRXNNotify("Fly OFF")
	end
	...
	local function MRXN_SetFreeze(on)
		...
		if on then
			local r = GetRCRoot()
			if r then _FrozenCF = r.CFrame end
	...
	local function MRXN_TeleportTo(name)
		...
		if not found or not found.Character then
			MRXNNotify("Tidak ketemu: " .. name) return
		end
		...
		if not tr then MRXNNotify("Karakter target tidak punya root") return end
	...
	-- ════════════════════════════════════════════════════════════
	--  UI — menyatu dengan menu Uhhhhhh
	-- ════════════════════════════════════════════════════════════

	-- ── DANCE PAGE ─────────────────────────────────────────────
	local DancePage = UI.CreateItemListPage()
	DancePage.ZIndex      = 1
	DancePage.Position    = UDim2.new(0.5, 360, 0.5, 0)
	DancePage.Interactable = false
	DancePage.Visible     = false

	local function OpenPage(page)
		page.Interactable  = false
		page.Visible       = true
		MainPage.Interactable = false
		local tw = TweenService:Create(page, TweenInfo.new(0.45, Enum.EasingStyle.Cubic, Enum.EasingDirection.In), {
			Position = UDim2.new(0.5, 0, 0.5, 0)
		})
		tw:Play()
	...
	local function ClosePage(page)
		page.Interactable = false
		local tw = TweenService:Create(page, TweenInfo.new(0.45, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {
			Position = UDim2.new(0.5, 360, 0.5, 0)
		})
		tw:Play()
	...
	UI.CreateText(DancePage, "Dance Presets  (terlihat semua orang)", 13, Enum.TextXAlignment.Center)
	...
	for _, preset in MRXN_DancePresets do
		...
		UI.CreateText(item, "rbxassetid://" .. p.Id, 11, Enum.TextXAlignment.Left)
		item.Activated:Connect(function()
			MRXN_PlayDance(p.Id)
			ClosePage(DancePage)
		end)
	...
	TpPage.ZIndex      = 1
	TpPage.Position    = UDim2.new(0.5, 360, 0.5, 0)
	TpPage.Interactable = false
	TpPage.Visible     = false

	local function RebuildTpList()
		for _, c in TpPage.List:GetChildren() do
		...
		for _, p in Players:GetPlayers() do
			if p ~= Player then
				...
				UI.CreateText(item, p.DisplayName, 15, Enum.TextXAlignment.Left)
				UI.CreateText(item, "@" .. p.Name, 11, Enum.TextXAlignment.Left)
				item.Activated:Connect(function()
					MRXN_TeleportTo(p.Name)
					ClosePage(TpPage)
				end)
			end
		end
		if count == 0 then
			UI.CreateText(TpPage.List, "Tidak ada player lain", 14, Enum.TextXAlignment.Center)
		end
	end

	UI.CreateButton(TpPage, "< Kembali", 18).Activated:Connect(function() ClosePage(TpPage) end)
	UI.CreateText(TpPage, "Pilih Player — Teleport Instan", 13, Enum.TextXAlignment.Center)
	UI.CreateSeparator(TpPage)

	-- ── MAIN PAGE — section MRXN ───────────────────────────────
	UI.CreateSeparator(MainPage)
	...
	local animBox = UI.CreateTextbox(MainPage, "", "AnimationId (angka saja)...", 16)

	UI.CreateButton(MainPage, "▶ Play AnimId Custom", 18).Activated:Connect(function()
		local id = animBox.Text:match("%d+")
		if id then MRXN_PlayDance(id)
		else MRXNNotify("Isi AnimationId dulu!") end
	end)

	UI.CreateButton(MainPage, "■ Stop Dance", 18).Activated:Connect(function()
		MRXN_StopDance()
		MRXNNotify("Dance OFF")
	end)
	...
	local danceSpeedSlider = UI.CreateSlider(MainPage, "Dance Speed", MRXN_Dance.Speed, 0.1, 3, 0.1)
	danceSpeedSlider.Changed:Connect(function(v)
		MRXN_Dance.Speed = v
		if MRXN_Dance.Track and MRXN_Dance.Track.IsPlaying then
			pcall(function() MRXN_Dance.Track:AdjustSpeed(v) end)
		end
	end)
	...
	UI.CreateSwitch(MainPage, "Fly Mode (E naik, Q turun)", false).Changed:Connect(function(v)
		if v then MRXN_StartFly() else MRXN_StopFly() end
	end)

	UI.CreateSlider(MainPage, "Fly Speed", MRXN_Move.FlySpeed, 5, 500, 5).Changed:Connect(function(v)
		MRXN_Move.FlySpeed = v
	end)

	UI.CreateSwitch(MainPage, "Noclip (tembus semua)", false).Changed:Connect(function(v)
		MRXN_SetNoclip(v)
	end)

	UI.CreateSwitch(MainPage, "Freeze Posisi", false).Changed:Connect(function(v)
		MRXN_SetFreeze(v)
	end)

	UI.CreateSlider(MainPage, "Walk Speed  (x16)", 1, 1, 20, 0.5).Changed:Connect(function(v)
		MRXN_SetSpeed(v)
	end)
	...
	UI.CreateButton(MainPage, "⚡ Teleport ke Player", 18).Activated:Connect(function()
		if tpBox.Text ~= "" then
			MRXN_TeleportTo(tpBox.Text)
		else
			MRXNNotify("Isi nama player dulu!")
		end
	end)
	...
	UI.CreateButton(MainPage, "↩ Teleport ke Spawn (0,0,0)", 18).Activated:Connect(function()
	...
	UI.CreateSeparator(MainPage)
	UI.CreateText(MainPage, "MRXN Patch v2.0  |  @Ghost", 11, Enum.TextXAlignment.Right)

	-- Expose ke _G
	_G_Uhhhhhh.MRXN = {
		PlayDance  = MRXN_PlayDance,
		StopDance  = MRXN_StopDance,
		StartFly   = MRXN_StartFly,
		StopFly    = MRXN_StopFly,
		SetNoclip  = MRXN_SetNoclip,
		SetFreeze  = MRXN_SetFreeze,
		SetSpeed   = MRXN_SetSpeed,
		TeleportTo = MRXN_TeleportTo,
	}

	MRXNNotify("MRXN v2.0 siap! Scroll bawah di menu.")
end)
