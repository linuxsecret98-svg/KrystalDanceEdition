...
-- ╔══════════════════════════════════════════════════════════════╗
-- ║  MRXN PATCH v3.0 — Built-in ke Uhhhhhh Reanimate            ║
-- ║  Fix: dance hilang sebentar, player baru join tidak lihat    ║
-- ╚══════════════════════════════════════════════════════════════╝
...
task.spawn(function()
	...
	-- ════════════════════════════════════════════════════════════
	--  DANCE VISIBLE — v3 rewrite
	--
	--  Root cause v2 gagal:
	--   • track.Stopped tidak selalu fire di semua game
	--   • Guard loop wait(2) terlalu lambat → gap 2 detik keliatan
	--   • MRXN_LoadAndPlayTrack rekursif lewat Stopped event →
	--     bisa stack overflow kalau game agresif stop track
	--
	--  Fix v3 — satu Heartbeat loop untuk SEGALANYA:
	--   • Tiap frame: cek track.IsPlaying
	--   • Kalau tidak playing → langsung Play() lagi TANPA reload
	--     (track Roblox bisa di-Play ulang walau sudah stop)
	--   • Kalau track sudah expired/destroyed → buat track baru
	--   • Sync Motor6D jalan di loop yang sama → zero overhead
	--   • Tidak ada event-driven restart, tidak ada wait/delay
	-- ════════════════════════════════════════════════════════════
	...
	local MRXN_Dance = {
		...
		Track   = nil,
		Speed   = 1,
		MainConn = nil,   -- satu Heartbeat connection untuk segalanya
	}
	...
	-- Buat track baru, bypass hook Uhhhhhh
	...
	local function MRXN_PlayDance(animId)
		...
		-- ── SATU LOOP UNTUK SEGALANYA ──────────────────────────
		-- Jalan tiap frame selama Active = true.
		-- Tidak ada event, tidak ada delay, tidak ada rekursi.
		MRXN_Dance.MainConn = RunService.Heartbeat:Connect(function()
			if not MRXN_Dance.Active then
				-- kalau di-stop dari luar, putus sendiri
				if MRXN_Dance.MainConn then
					MRXN_Dance.MainConn:Disconnect()
					MRXN_Dance.MainConn = nil
				end
			...
			-- 1) Pastikan track tetap playing
			local curTrack = MRXN_Dance.Track
			if curTrack then
				-- cek apakah track masih valid (tidak destroyed)
				local trackOk = pcall(function()
					if not curTrack.IsPlaying then
						curTrack:Play(0, 1, MRXN_Dance.Speed)
					end
				end)
				-- kalau track destroyed/invalid, buat yang baru
				if not trackOk then
					local newTr = MRXN_MakeTrack(MRXN_Dance.AnimId)
					if newTr then
						newTr:Play(0, 1, MRXN_Dance.Speed)
						MRXN_Dance.Track = newTr
					end
				end
			else
				-- track nil, buat baru
			...
			-- 2) Sync Motor6D: copy pose real char → reanimate char
			--    Ini yang bikin orang lain di server lihat dance kamu,
			--    termasuk player yang baru join server
			local rc = GetRC()
			local rl = GetRL()
			if rc and rl then
	...
	local function MRXN_StopFly()
		MRXN_Move.FlyEnabled = false; MRXN_KillFly()
		MRXNNotify("Fly OFF")
	end
	...
	UI.CreateButton(MainPage,"⚡ Teleport ke Player",18).Activated:Connect(function()
		if tpBox.Text~="" then MRXN_TeleportTo(tpBox.Text) else MRXNNotify("Isi nama player!") end
	end)
	...
	UI.CreateSeparator(MainPage)
	UI.CreateText(MainPage,"MRXN v3.0  |  @Ghost",11,Enum.TextXAlignment.Right)
	...
	MRXNNotify("v3.0 loaded! Scroll bawah.")
end)
-- ╚══════════════════════ END MRXN v3.0 ════════════════════════╝
