...
-- ══════════════════════════════════════════════════════════════
--  MRXH UPGRADE: Global Animation Track Killer
--  Intercepts AnimationTrack:Play() calls from the game via
--  __namecall hook so that even games with many layered
--  animations (idle, walk, run, emote, etc.) cannot override
--  the reanimate pose visible to other players on the server.
-- ══════════════════════════════════════════════════════════════
local AnimTrackKiller = {}
AnimTrackKiller._active = false
AnimTrackKiller._nc_old = nil
AnimTrackKiller._dummy = Instance.new("AnimationTrack") -- for hookmetamethod target

local function _IsPlayerCharTrack(self)
	-- Only block tracks that belong to the local player's real character
	if typeof(self) ~= "Instance" then return false end
	if not self:IsA("AnimationTrack") then return false end
	local char = Player.Character
	if not char then return false end
	-- Walk up the track's animator parent chain
	local ok, anim = pcall(function() return self.Animation end)
	if not ok then return false end
	-- Check if any Animator ancestor is inside Player.Character
	local inst = self
	for _ = 1, 10 do
		inst = inst.Parent
		if inst == nil then return false end
		if inst == char then return true end
	end
	return false
end

if not ismissing(hookmetamethod) and not ismissing(newcclosure) and not ismissing(checkcaller) then
	local _dummy_track = nil
	pcall(function()
		-- We need a real AnimationTrack instance for hookmetamethod
		local tmpChar = Instance.new("Model")
		local tmpHum = Instance.new("Humanoid")
		tmpHum.Parent = tmpChar
		tmpChar.Parent = workspace
		local tmpAnim = Instance.new("Animation")
		tmpAnim.AnimationId = "rbxassetid://507770453"
		local tmpAnimator = Instance.new("Animator")
		tmpAnimator.Parent = tmpHum
		local track = tmpAnimator:LoadAnimation(tmpAnim)
		_dummy_track = track
		tmpChar:Destroy()
	end)

	if _dummy_track then
		local _nc_orig = nil
		local _newNC = newcclosure(function(self, ...)
			local method = getnamecallmethod()
			if not checkcaller() and method == "Play" then
				if typeof(self) == "Instance" and pcall(function() return self:IsA("AnimationTrack") end) and self:IsA("AnimationTrack") then
					if AnimTrackKiller._active then
						local char = Player.Character
						if char then
							-- Check if this track's animator is inside Player.Character
							local ok, parent = pcall(function()
								local p = self.Parent
								for _ = 1, 8 do
									if p == nil then return nil end
									if p == char then return char end
									p = p.Parent
								end
								return nil
							end)
							if ok and parent == char then
								-- Block the Play() call and immediately stop
								pcall(function() self:Stop(0) end)
								return
							end
						end
					end
			...
			return _nc_orig(self, ...)
		end)
		pcall(function()
			_nc_orig = hookmetamethod(_dummy_track, "__namecall", _newNC)
			AnimTrackKiller._nc_old = _nc_orig
		end)
	end
end

function AnimTrackKiller.SetActive(state)
	AnimTrackKiller._active = state
end

-- Aggressive sweeper: stop ALL playing tracks on character every frame
function AnimTrackKiller.SweepCharacter(character)
...
function LimbReanimator.Start()
	...
	local CharOnDesc = function(v)
		...
		elseif v:IsA("Animator") then
			-- Stop semua track dulu sebelum di-destroy supaya animasi game tidak override reanimate
			pcall(function()
	...
	local CharConn = Player.CharacterAdded:Connect(function(character)
		...
		-- Stop semua animation track yang sedang berjalan agar reanimate keliatan di server
		local stupid = humanoid:FindFirstChildWhichIsA("Animator")
		if stupid then
			pcall(function()
				for _, track in stupid:GetPlayingAnimationTracks() do
					track:Stop(0)
				end
			end)
			stupid:Destroy()
		end
		-- Juga stop lewat humanoid langsung (fallback)
		pcall(function()
		...
		-- Persistent killer: pastikan tidak ada animasi baru yang diputar game tiap frame
		Util.LinkDestroyI2C(character, RunService.PreAnimation:Connect(function()
			if not character or not character.Parent then return end
			local hum = character:FindFirstChildOfClass("Humanoid")
			if not hum then return end
			local anim2 = hum:FindFirstChildOfClass("Animator")
			if anim2 then
				pcall(function()
					for _, track in anim2:GetPlayingAnimationTracks() do
						track:Stop(0)
					end
				end)
			end
	...
	-- MRXH: Activate global animation track killer for this reanimate session
	AnimTrackKiller.SetActive(true)
	...
	while not Reanimate.Stopping do
		...
		if Character then
			-- MRXH: Aggressive sweep — kill every playing track every frame
			AnimTrackKiller.SweepCharacter(Character)
	...
	CharConn:Disconnect()
	-- MRXH: Deactivate animation track killer when reanimate stops
	AnimTrackKiller.SetActive(false)
	if Player.Character then
...
function HatReanimator.Start()
	...
	local CharOnDesc = function(v)
		...
		elseif v:IsA("Animator") then
			-- Stop semua track dulu sebelum di-destroy
			pcall(function()
				for _, track in v:GetPlayingAnimationTracks() do
					track:Stop(0)
				end
	...
	Reanimate.Starting = false
	local letitgo = 0
	-- MRXH: Activate animation track killer for HatReanimator session
	AnimTrackKiller.SetActive(true)
	...
	while not Reanimate.Stopping do
		...
		if Character and Character:IsDescendantOf(workspace) then
			-- MRXH: Kill all playing animation tracks every frame
			AnimTrackKiller.SweepCharacter(Character)
	...
	for _,v in HatRefs do if v.PH then v.PH:Destroy() end end
	CharConn:Disconnect()
	-- MRXH: Deactivate animation track killer when HatReanimator stops
	AnimTrackKiller.SetActive(false)
	--replicatesignal(Player.ConnectDiedSignalBackend)
	Reanimate.Stopping = false
	Reanimate.DestroyCharacter()
...
-- ╔══════════════════════════════════════════════════════════════╗
-- ║  MRXN PATCH v4.0 — Built-in ke Uhhhhhh Reanimate            ║
-- ║  + RootPart Mode shortcut semua 5 mode                       ║
-- ╚══════════════════════════════════════════════════════════════╝

task.spawn(function()
	repeat task.wait(0.2) until IsUhhhhhhFullyLoaded

	-- ════════════════════════
	--  UTILS
	-- ════════════════════════
	local function MRXNNotify(msg) Util.UINotify("[MRXN] " .. msg) end
	local function GetRC()     return Reanimate.Character end
	local function GetRL()     return Player.Character end
	local function GetRCRoot() local c=GetRC() return c and c:FindFirstChild("HumanoidRootPart") end
	local function GetRLRoot() local c=GetRL() return c and c:FindFirstChild("HumanoidRootPart") end
	local function GetRLHum()
		local c=GetRL(); if not c then return nil end
		return c:FindFirstChildOfClass("Humanoid") or c:FindFirstChild("Humanoid",true)
	end
	local function GetRCHum()
		local c=GetRC(); if not c then return nil end
		return c:FindFirstChildOfClass("Humanoid") or c:FindFirstChild("Humanoid",true)
	end

	-- ════════════════════════════════════════════════════════════
	--  ROOTPART MODE — shortcut langsung ubah LimbReanimator.Mode
	--
	--  Mode 0 = RootPart in very void   (default, paling aman)
	--  Mode 1 = RootPart in void        (sedikit lebih dekat)
	--  Mode 2 = Keep RootPart Streamed  (paksa di sekitar char)
	--  Mode 3 = CurrentAngle Style      (ikut posisi RCRootPart)
	--  Mode 4 = RootPart is Torso       (ikut Torso, paling smooth)
	--
	--  Perubahan mode langsung apply ke LimbReanimator.Mode
	--  yang dibaca tiap frame oleh loop utama Uhhhhhh —
	--  tidak perlu restart reanimate.
	-- ════════════════════════════════════════════════════════════

	local MRXN_RootModes = {
		{ Name = "In Very Void",         Mode = 0 },
		{ Name = "In Void",              Mode = 1 },
		{ Name = "Keep Streamed",        Mode = 2 },
		{ Name = "CurrentAngle Style",   Mode = 3 },
		{ Name = "RootPart is Torso",    Mode = 4 },
	}

	local function MRXN_SetRootMode(mode)
		LimbReanimator.Mode = mode
		SaveData.Reanimator = SaveData.Reanimator or {}
		SaveData.Reanimator.LimbMode = mode
		local names = {"Very Void","Void","Keep Streamed","CurrentAngle","Torso"}
		MRXNNotify("RootPart Mode → " .. (names[mode+1] or tostring(mode)))
	end

	-- ════════════════════════════════════════════════════════════
	--  DANCE VISIBLE v3 — satu Heartbeat loop untuk segalanya
	-- ════════════════════════════════════════════════════════════
	...
	local MRXN_DancePresets = {
		{ Name = "Wave",           Id = "507770239"  },
		{ Name = "Dance",          Id = "507771019"  },
		{ Name = "Robot",          Id = "507776879"  },
		{ Name = "Gangnam Style",  Id = "182435998"  },
		{ Name = "Tilt",           Id = "507770552"  },
		{ Name = "Point",          Id = "507770453"  },
		{ Name = "Laugh",          Id = "507770818"  },
		{ Name = "Cheer",          Id = "507770677"  },
		{ Name = "Salute",         Id = "3337566660" },
		{ Name = "Breakdance",     Id = "5001374049" },
		{ Name = "Floss",          Id = "3457881641" },
		{ Name = "Wiggle",         Id = "3905474773" },
		{ Name = "Samba",          Id = "3905476914" },
		{ Name = "Superstar",      Id = "5898610849" },
		{ Name = "Hype",           Id = "6526550285" },
		{ Name = "Orange Justice", Id = "4867863518" },
		{ Name = "Griddy",         Id = "12785615993"},
	}

	local function MRXN_MakeTrack(animId)
		local hum = GetRLHum()
	...
	local function MRXN_StopDance()
		MRXN_Dance.Active = false
		if MRXN_Dance.MainConn then
			MRXN_Dance.MainConn:Disconnect()
			MRXN_Dance.MainConn = nil
		end
		if MRXN_Dance.Track then
			pcall(function() MRXN_Dance.Track:Stop(0.15) end)
			MRXN_Dance.Track = nil
		end
	end

	local function MRXN_PlayDance(animId)
		MRXN_StopDance()
		if not GetRLHum() then MRXNNotify("Reanimate dulu!") return false end

		local id = tostring(animId)
		MRXN_Dance.AnimId  = id
		MRXN_Dance.Active  = true

		local tr = MRXN_MakeTrack(id)
		if not tr then
			MRXN_Dance.Active = false
			MRXNNotify("Gagal load: " .. id)
			return false
		end
		tr:Play(0.15, 1, MRXN_Dance.Speed)
		MRXN_Dance.Track = tr

		-- Satu Heartbeat loop: cek track + sync Motor6D tiap frame
		MRXN_Dance.MainConn = RunService.Heartbeat:Connect(function()
			if not MRXN_Dance.Active then
				if MRXN_Dance.MainConn then
					MRXN_Dance.MainConn:Disconnect()
					MRXN_Dance.MainConn = nil
				end
			...
			-- 1) Pastikan track tetap playing — cek tiap frame, bukan pakai event
			local cur = MRXN_Dance.Track
			if cur then
				local ok = pcall(function()
					if not cur.IsPlaying then
						cur:Play(0, 1, MRXN_Dance.Speed)
					end
				end)
				if not ok then
					-- track invalid/destroyed → buat baru
					local nt = MRXN_MakeTrack(MRXN_Dance.AnimId)
					if nt then nt:Play(0, 1, MRXN_Dance.Speed) end
					MRXN_Dance.Track = nt
				end
			else
				local nt = MRXN_MakeTrack(MRXN_Dance.AnimId)
			...
			-- 2) Sync Motor6D real char → reanimate char tiap frame
			--    Player baru join langsung lihat pose ini karena sync permanen
			local rc = GetRC()
			local rl = GetRL()
			if rc and rl then
				for _, motor in rl:GetDescendants() do
					if motor:IsA("Motor6D") and motor.Part0 and motor.Part1 then
						local rm = rc:FindFirstChild(motor.Name, true)
						if rm and rm:IsA("Motor6D") then
							pcall(function() rm.Transform = motor.Transform end)
						end
					end
				end
			end
	...
	--  POSISI BEBAS
	-- ════════════════════════════════════════════════════════════
	local MRXN_Move = { FlyEnabled=false, FlySpeed=50, NoclipEnabled=false, Frozen=false }
	local _FlyBV, _FlyBG, _FlyConn, _NoclipConn, _FreezeConn, _FrozenCF

	local function MRXN_KillFly()
	...
	local function MRXN_StartFly()
		MRXN_KillFly()
		local root = GetRCRoot() or GetRLRoot()
		if not root then MRXNNotify("Reanimate dulu!") return end
		local bv = Instance.new("BodyVelocity", root)
		bv.MaxForce = Vector3.new(1e9,1e9,1e9); bv.Velocity = Vector3.zero
		_FlyBV = bv
		local bg = Instance.new("BodyGyro", root)
		bg.MaxTorque = Vector3.new(1e9,1e9,1e9); bg.P=9000; bg.D=100
		_FlyBG = bg
		_FlyConn = RunService.RenderStepped:Connect(function()
			if not MRXN_Move.FlyEnabled then MRXN_KillFly() return end
			local r = GetRCRoot() or GetRLRoot(); if not r then return end
			if bv.Parent~=r then bv.Parent=r end
			if bg.Parent~=r then bg.Parent=r end
			local dir=Vector3.zero; local cf=Camera.CFrame
			local ks={}; for _,k in UserInputService:GetKeysPressed() do ks[k.KeyCode]=true end
			if ks[Enum.KeyCode.W] then dir+=cf.LookVector  end
			if ks[Enum.KeyCode.S] then dir-=cf.LookVector  end
			if ks[Enum.KeyCode.A] then dir-=cf.RightVector end
			if ks[Enum.KeyCode.D] then dir+=cf.RightVector end
			if ks[Enum.KeyCode.E] or ks[Enum.KeyCode.Space]      then dir+=Vector3.yAxis end
			if ks[Enum.KeyCode.Q] or ks[Enum.KeyCode.LeftShift] then dir-=Vector3.yAxis end
			bv.Velocity = dir.Magnitude>0 and dir.Unit*MRXN_Move.FlySpeed or Vector3.zero
			bg.CFrame   = cf
		end)
		MRXN_Move.FlyEnabled = true
		MRXNNotify("Fly ON — E naik, Q turun")
	end

	local function MRXN_StopFly()
		MRXN_Move.FlyEnabled=false; MRXN_KillFly(); MRXNNotify("Fly OFF")
	end

	local function MRXN_SetNoclip(on)
		MRXN_Move.NoclipEnabled=on
		if _NoclipConn then _NoclipConn:Disconnect(); _NoclipConn=nil end
		if on then
			_NoclipConn = RunService.Stepped:Connect(function()
				for _,char in {GetRC(),GetRL()} do
					if char then for _,p in char:GetDescendants() do
						if p:IsA("BasePart") then p.CanCollide=false end
					end end
				end
			end)
			MRXNNotify("Noclip ON")
		else MRXNNotify("Noclip OFF") end
	end

	local function MRXN_SetFreeze(on)
		MRXN_Move.Frozen=on
		if _FreezeConn then _FreezeConn:Disconnect(); _FreezeConn=nil end
		if on then
			local r=GetRCRoot(); if r then _FrozenCF=r.CFrame end
			_FreezeConn = RunService.Heartbeat:Connect(function()
				local r2=GetRCRoot()
				if r2 and _FrozenCF then
					r2.CFrame=_FrozenCF
					r2.AssemblyLinearVelocity=Vector3.zero
					r2.AssemblyAngularVelocity=Vector3.zero
				end
			end)
			MRXNNotify("Freeze ON")
		else MRXNNotify("Freeze OFF") end
	end

	local function MRXN_SetSpeed(mul)
		local rh=GetRCHum(); if rh then rh.WalkSpeed=16*mul end
		local rl=GetRLHum(); if rl then rl.WalkSpeed=16*mul end
		MRXNNotify("Speed "..mul.."x")
	end

	local function MRXN_TeleportTo(name)
		local found=nil
		for _,p in Players:GetPlayers() do
			if p~=Player and p.Name:lower():find(name:lower(),1,true) then found=p; break end
		end
		if not found or not found.Character then MRXNNotify("Tidak ketemu: "..name) return end
		local tr = found.Character:FindFirstChild("HumanoidRootPart")
		         or found.Character:FindFirstChild("LowerTorso")
		         or found.Character:FindFirstChild("Torso")
		if not tr then MRXNNotify("Target tidak punya root") return end
		local dest = tr.CFrame * CFrame.new(0,0,3)
		local rc=GetRCRoot(); if rc then rc.CFrame=dest; rc.AssemblyLinearVelocity=Vector3.zero end
		local rl=GetRLRoot(); if rl then rl.CFrame=dest; rl.AssemblyLinearVelocity=Vector3.zero end
		MRXNNotify("TP → "..found.DisplayName)
	end
	...
	local function OpenPage(page)
		page.Interactable=false; page.Visible=true; MainPage.Interactable=false
		local tw=TweenService:Create(page,TweenInfo.new(0.4,Enum.EasingStyle.Cubic,Enum.EasingDirection.In),{Position=UDim2.new(0.5,0,0.5,0)})
		tw:Play(); tw.Completed:Connect(function() page.Interactable=true end)
	end
	local function ClosePage(page)
		page.Interactable=false
		local tw=TweenService:Create(page,TweenInfo.new(0.4,Enum.EasingStyle.Cubic,Enum.EasingDirection.Out),{Position=UDim2.new(0.5,360,0.5,0)})
		tw:Play(); tw.Completed:Connect(function() MainPage.Interactable=true; page.Visible=false end)
	end

	-- ── RootPart Mode Page ─────────────────────────────────────
	local RootPage = UI.CreateItemListPage()
	RootPage.ZIndex=1; RootPage.Position=UDim2.new(0.5,360,0.5,0)
	RootPage.Interactable=false; RootPage.Visible=false
	UI.CreateButton(RootPage,"< Kembali",18).Activated:Connect(function() ClosePage(RootPage) end)
	UI.CreateText(RootPage,"RootPart Mode — apply langsung",14,Enum.TextXAlignment.Center)
	UI.CreateText(RootPage,"Tidak perlu restart reanimate",11,Enum.TextXAlignment.Center)
	UI.CreateSeparator(RootPage)

	-- Deskripsi singkat tiap mode
	local modeDesc = {
		[0] = "RootPart dibuang ke koordinat sangat jauh di bawah map (paling aman, default)",
		[1] = "RootPart dibuang ke dekat void, sedikit lebih dekat dari mode 0",
		[2] = "RootPart dijaga tetap streamed di bawah karakter (perlu streaming aktif)",
		[3] = "RootPart mengikuti posisi/rotasi RCRootPart (CurrentAngle style)",
		[4] = "RootPart melekat ke Torso — paling smooth & paling banyak direplikasi",
	}

	for _, m in MRXN_RootModes do
		local mdata = m
		local item = UI.CreateItemListItem(RootPage.List)
		UI.CreateText(item, mdata.Name, 16, Enum.TextXAlignment.Left)
		UI.CreateText(item, modeDesc[mdata.Mode], 10, Enum.TextXAlignment.Left)
		item.Activated:Connect(function()
			MRXN_SetRootMode(mdata.Mode)
			ClosePage(RootPage)
		end)
	end

	-- ── Dance Page ─────────────────────────────────────────────
	local DancePage = UI.CreateItemListPage()
	DancePage.ZIndex=1; DancePage.Position=UDim2.new(0.5,360,0.5,0)
	DancePage.Interactable=false; DancePage.Visible=false
	UI.CreateButton(DancePage,"< Kembali",18).Activated:Connect(function() ClosePage(DancePage) end)
	UI.CreateText(DancePage,"Dance Presets",14,Enum.TextXAlignment.Center)
	UI.CreateSeparator(DancePage)
	for _, preset in MRXN_DancePresets do
		local p=preset
		local item=UI.CreateItemListItem(DancePage.List)
		UI.CreateText(item,p.Name,16,Enum.TextXAlignment.Left)
		UI.CreateText(item,"ID: "..p.Id,11,Enum.TextXAlignment.Left)
		item.Activated:Connect(function() MRXN_PlayDance(p.Id); ClosePage(DancePage) end)
	end

	-- ── Teleport Page ──────────────────────────────────────────
	local TpPage = UI.CreateItemListPage()
	TpPage.ZIndex=1; TpPage.Position=UDim2.new(0.5,360,0.5,0)
	TpPage.Interactable=false; TpPage.Visible=false
	local function RebuildTp()
		for _,c in TpPage.List:GetChildren() do if c:IsA("GuiObject") then c:Destroy() end end
		local n=0
		for _,p in Players:GetPlayers() do
			if p~=Player then
				n+=1
				local item=UI.CreateItemListItem(TpPage.List)
				UI.CreateText(item,p.DisplayName.." (@"..p.Name..")",15,Enum.TextXAlignment.Left)
				item.Activated:Connect(function() MRXN_TeleportTo(p.Name); ClosePage(TpPage) end)
			end
		end
		if n==0 then UI.CreateText(TpPage.List,"Tidak ada player lain",14,Enum.TextXAlignment.Center) end
	end
	UI.CreateButton(TpPage,"< Kembali",18).Activated:Connect(function() ClosePage(TpPage) end)
	UI.CreateText(TpPage,"Pilih Player → Teleport",13,Enum.TextXAlignment.Center)
	UI.CreateSeparator(TpPage)

	-- ── Main Page ──────────────────────────────────────────────
	UI.CreateSeparator(MainPage)
	UI.CreateText(MainPage,"▼  MRXN EXTRAS  ▼",15,Enum.TextXAlignment.Center)
	UI.CreateSeparator(MainPage)

	-- RootPart Mode
	UI.CreateText(MainPage,"◈ ROOTPART MODE",13,Enum.TextXAlignment.Left)
	-- Dropdown langsung di main page (sync dengan LimbReanimator)
	local rootDrop = UI.CreateDropdown(MainPage, "Mode Aktif",
		{"In Very Void","In Void","Keep Streamed","CurrentAngle Style","RootPart is Torso"},
		LimbReanimator.Mode + 1
	)
	rootDrop.Changed:Connect(function(val)
		MRXN_SetRootMode(val - 1)
	end)
	-- Sync dropdown kalau LimbReanimator.Mode diubah dari UI lain
	RunService.Heartbeat:Connect(function()
		if rootDrop.Value ~= LimbReanimator.Mode + 1 then
			rootDrop.Value = LimbReanimator.Mode + 1
		end
	end)
	UI.CreateButton(MainPage,"Detail & Deskripsi Mode  ▶",18).Activated:Connect(function()
		OpenPage(RootPage)
	end)

	-- Dance
	UI.CreateSeparator(MainPage)
	UI.CreateText(MainPage,"◈ DANCE VISIBLE",13,Enum.TextXAlignment.Left)
	local animBox=UI.CreateTextbox(MainPage,"","AnimationId (angka)...",16)
	UI.CreateButton(MainPage,"▶ Play AnimId",18).Activated:Connect(function()
		local id=animBox.Text:match("%d+")
		if id then MRXN_PlayDance(id) else MRXNNotify("Isi AnimationId dulu!") end
	end)
	UI.CreateButton(MainPage,"■ Stop Dance",18).Activated:Connect(function()
		MRXN_StopDance(); MRXNNotify("Dance OFF")
	end)
	UI.CreateButton(MainPage,"Pilih Dance Preset  ▶",18).Activated:Connect(function()
		OpenPage(DancePage)
	end)
	UI.CreateSlider(MainPage,"Dance Speed",MRXN_Dance.Speed,0.1,3,0.1).Changed:Connect(function(v)
		MRXN_Dance.Speed=v
		if MRXN_Dance.Track then pcall(function() MRXN_Dance.Track:AdjustSpeed(v) end) end
	end)

	-- Move
	UI.CreateSeparator(MainPage)
	UI.CreateText(MainPage,"◈ POSISI BEBAS",13,Enum.TextXAlignment.Left)
	UI.CreateSwitch(MainPage,"Fly (E naik, Q turun)",false).Changed:Connect(function(v)
		if v then MRXN_StartFly() else MRXN_StopFly() end
	end)
	UI.CreateSlider(MainPage,"Fly Speed",50,5,500,5).Changed:Connect(function(v)
		MRXN_Move.FlySpeed=v
	end)
	UI.CreateSwitch(MainPage,"Noclip",false).Changed:Connect(function(v) MRXN_SetNoclip(v) end)
	UI.CreateSwitch(MainPage,"Freeze Posisi",false).Changed:Connect(function(v) MRXN_SetFreeze(v) end)
	UI.CreateSlider(MainPage,"Walk Speed (x16)",1,1,20,0.5).Changed:Connect(function(v) MRXN_SetSpeed(v) end)

	-- Teleport
	UI.CreateSeparator(MainPage)
	UI.CreateText(MainPage,"◈ TELEPORT",13,Enum.TextXAlignment.Left)
	local tpBox=UI.CreateTextbox(MainPage,"","Nama player...",16)
	UI.CreateButton(MainPage,"⚡ Teleport ke Player",18).Activated:Connect(function()
		if tpBox.Text~="" then MRXN_TeleportTo(tpBox.Text) else MRXNNotify("Isi nama dulu!") end
	end)
	UI.CreateButton(MainPage,"Pilih dari Daftar  ▶",18).Activated:Connect(function()
		RebuildTp(); OpenPage(TpPage)
	end)
	UI.CreateButton(MainPage,"↩ TP ke 0,0,0",18).Activated:Connect(function()
		local dest=CFrame.new(0,5,0)
		local rc=GetRCRoot(); if rc then rc.CFrame=dest; rc.AssemblyLinearVelocity=Vector3.zero end
		local rl=GetRLRoot(); if rl then rl.CFrame=dest; rl.AssemblyLinearVelocity=Vector3.zero end
		MRXNNotify("TP → origin")
	end)

	UI.CreateSeparator(MainPage)
	UI.CreateText(MainPage,"MRXN v4.0  |  @Ghost",11,Enum.TextXAlignment.Right)

	_G_Uhhhhhh.MRXN = {
		PlayDance=MRXN_PlayDance, StopDance=MRXN_StopDance,
		StartFly=MRXN_StartFly,   StopFly=MRXN_StopFly,
		SetNoclip=MRXN_SetNoclip, SetFreeze=MRXN_SetFreeze,
		SetSpeed=MRXN_SetSpeed,   TeleportTo=MRXN_TeleportTo,
		SetRootMode=MRXN_SetRootMode,
	}

	MRXNNotify("v4.0 loaded! Scroll bawah di menu.")
end)
-- ╚══════════════════════ END MRXN v4.0 ════════════════════════╝
