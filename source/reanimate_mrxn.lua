...
task.spawn(function()
	-- Tunggu sampai Uhhhhhh fully loaded
	...
	-- ════════════════════════════════════════════════
	--  MODUL: DANCE VISIBLE (diputar di real character)
	-- ════════════════════════════════════════════════
	local MRXN_Dance = {
		Active = false,
		AnimId = "",
	...
	local function MRXN_PlayDance(animId, loop, speed)
		...
		if not realHum then MRXNNotify("Butuh reanimate aktif!") return false end

		local anim = Instance.new("Animation")
		anim.AnimationId = "rbxassetid://" .. tostring(animId)
		local ok, track = pcall(function() return realHum:LoadAnimation(anim) end)
		anim:Destroy()
		if not ok or not track then MRXNNotify("Gagal load anim: " .. tostring(animId)) return false end

		track.Looped   = (loop ~= false)
		track.Priority = Enum.AnimationPriority.Action4
		track:Play(0.3, 1, speed or MRXN_Dance.Speed)

		MRXN_Dance.Track  = track
		MRXN_Dance.AnimId = tostring(animId)
		MRXN_Dance.Active = true

		-- Sync Motor6D fake char agar pose ikut di reanimate character
		task.spawn(function()
			while MRXN_Dance.Active and track.IsPlaying do
				local rc = Reanimate.Character
	...
	-- Tambah halaman baru MRXN ke MainPage
	UI.CreateSeparator(MainPage)
	...
	-- Preset dances — buat halaman sendiri
	...
	UI.CreateSeparator(MainPage)
	UI.CreateText(MainPage, "MRXN Patch by Ghost | v1.0", 11, Enum.TextXAlignment.Right)
